...
if __name__ == "__main__":
    ...
    def greet_person(
        person: Person,
        color: Annotated[Color, "Favorite color"],
        pet: Annotated[Optional[str], "Favorite pet"] = None,
    ) -> str:
        """
        Greet the person with their favorite color and pet.
        """
        greeting = f"Hello, {person.name}! Your favorite color is {color}."
        if pet:
            greeting += f" Your favorite pet is {pet}."
        return greeting

    # Retrieve the definition and function info
    tool_info = registry.registry["greet_person"]
    print(json.dumps(tool_info.definition, indent=2))

    # Call the function from JSON
    json_input = {
        "person": {"name": "Alice", "age": 30},
        "color": "red",
        "pet": "cat",
    }
    result = registry.call_tool("greet_person", json_input)
